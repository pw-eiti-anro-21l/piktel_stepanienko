...
class XYZ_RPY:
    ...
    def save(self):
        with open(os.path.join(get_package_share_directory('int_l4'), 'xyz_rpy.yaml'), 'w') as file:
            yaml.dump(self.xyz_rpy, file)

def readDH():
    with open(os.path.join(get_package_share_directory('int_l4'), 'joints.yaml'), 'r') as file:
        dhv = yaml.load(file, Loader=yaml.FullLoader)

    return dhv

def readLinks():
    with open(os.path.join(get_package_share_directory('int_l4'), 'links.yaml'), 'r') as file:
